...
class cuda_pISTA_PCD(base):
    def __init__(self, T, N, lam, ls_iter, step_lim, init_step):
        super(cuda_pISTA_PCD, self).__init__(T, N, lam)
        self.ls_iter = ls_iter
        self.step_lim = step_lim
        self.init_step = init_step
        self.save_name = "cuda_pISTA_PCD_N{N}_T{T}_step{step}_LsIter{ls_iter}_StepLim{step_lim}"\
    ...
    def compute(self, S, A0, status_f, history, test_check_f):
        ...
        for t in range(self.T):
            ...
            G = S - A_inv
            W = A_inv
            W_diag = cp.diag(W).copy().reshape(-1, 1)
            cp.fill_diagonal(W, 0)
            W_no_diag = W
            M = (W_no_diag * W_no_diag) + (W_diag * W_diag.T)
            cp.fill_diagonal(W, W_diag)
            div_M = 1.0 / M
            lam_div_M = lam * div_M
            M = None
            def PCD_iter(D):
                b = G + W.T @ D @ W
                b_div_M = b * div_M
                return cp_soft_threshold(cp, A + D - b_div_M, lam_div_M) - A - D
            #div_M = None
            #lam_div_M = None
...
def pista_cholesky_linesearch(cp, A, S, lam, mask, a, b, c, step, max_iter, step_lim, post_f):
    if step == 0:
        return A, 0.0
    beta = step
