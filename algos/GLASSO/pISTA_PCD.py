...
class pISTA_PCD(base):
    def __init__(self, T, N, lam, ls_iter, step_lim, init_step):
        super(pISTA_PCD, self).__init__(T, N, lam)
        self.ls_iter = ls_iter
        self.step_lim = step_lim
        self.init_step = init_step
        self.save_name = "pISTA_PCD_N{N}_T{T}_step{step}_LsIter{ls_iter}_StepLim{step_lim}"\
    ...
    def compute(self, S, A0, status_f, history, test_check_f):
        ...
        for t in range(self.T):
            ...
            G = S - A_inv
            W = A_inv
            W_diag = np.diag(W).copy().reshape(-1, 1)
            np.fill_diagonal(W, 0)
            W_no_diag = W
            M = (W_no_diag * W_no_diag) + (W_diag * W_diag.T)
            ...
            def PCD_iter(D):
                b = G + W.T @ D @ W
                b_div_M = b * div_M
                return np_soft_threshold(A + D - b_div_M, lam_div_M) - A - D
            #div_M = None
            #lam_div_M = None
...
def pista_cholesky_linesearch(A, S, lam, mask, a, b, c, step, max_iter, step_lim, post_f):
    if step == 0:
        return A, 0.0
    beta = step
...
def init_pISTA_PCD_parser(pISTA_PCD_pasrser):
    pISTA_PCD_pasrser.set_defaults(algo='pISTA_PCD')
    pISTA_PCD_pasrser.add_argument(
